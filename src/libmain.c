...
static void parse_command_line_options(gint *argc, gchar ***argv){
    ...
    if (! gtk_init_check(NULL, NULL)) {    /* check whether we have a valid X display and exit if not */
...
static gint create_config_dir(void) {
    ...
    if (saved_errno == 0 && ! g_file_test(conf_file, G_FILE_TEST_EXISTS)) {    /* check whether geany.conf can be written */
...
GEANY_EXPORT_SYMBOL
gint mainRun(gint argc, gchar **argv) {
    GeanyDocument *doc;
    ...
    ui_set_statusbar(TRUE, _("This is Greeny %s."), main_get_version_string());
...
static gboolean do_main_quit(void) {
    ...
    if (!document_close_all())        return FALSE;

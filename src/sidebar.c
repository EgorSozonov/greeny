...
/* callback prototypes */
static void on_openfiles_document_action(GtkMenuItem *menuitem, gpointer userData);
static gboolean sidebar_button_press_cb(GtkWidget *widget, GdkEventButton *event,
        gpointer userData);
static gboolean sidebar_key_press_cb(GtkWidget *widget, GdkEventKey *event, gpointer userData);
static void on_list_document_activate(GtkCheckMenuItem *item, gpointer userData);
static void on_list_symbol_activate(GtkCheckMenuItem *item, gpointer userData);
...
void sidebar_update_tag_list(GeanyDocument *doc, gboolean update) {
    ...
    if (doc->priv->tag_tree_dirty) {    /* updating the tag list in the left tag window */
...
static gboolean tree_foreach_callback(GtkTreeModel *model,
    ...
    gsize name_len;
    GeanyDocument *doc;
    TreeForeachData *data = (TreeForeachData*) userData;
...
void sidebar_openfiles_add(GeanyDocument *doc) {
    ...
    if (!file_icon) file_icon = ui_get_mime_icon("text/plain");
...
static void on_openfiles_show_paths_activate(GtkCheckMenuItem *item, gpointer userData) {
    gint new_mode = GPOINTER_TO_INT(userData);
...
static void on_find_in_files(GtkMenuItem *menuitem, gpointer userData) {
    GtkTreeSelection *treesel;
    GtkTreeIter iter;
    GtkTreeModel *model;
    GeanyDocument *doc;
...
static void on_openfiles_expand_collapse(GtkMenuItem *menuitem, gpointer userData) {
    gboolean expand = GPOINTER_TO_INT(userData);
...
static void on_openfiles_document_action(GtkMenuItem *menuitem, gpointer userData) {
    ...
    GtkTreeModel *model;
    gint action = GPOINTER_TO_INT(userData);
...
static gboolean sidebar_button_press_cb(GtkWidget *widget, GdkEventButton *event,
     ...
     * so guess it's fine. */
    if (widget_class->button_press_event) handled = widget_class->button_press_event(widget, event);
    ...
    if (event->type == GDK_2BUTTON_PRESS) { 
    ...
    } else if (event->button == 1) {    /* allow reclicking of taglist treeview item */
        ...
        if (widget == tv.tree_openfiles) {
            if (!openfiles_popup_menu) create_openfiles_popup_menu();
...
static void on_sidebar_switch_page(GtkNotebook *notebook,
    gpointer page, guint page_num, gpointer userData) {

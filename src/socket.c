...
static void handle_input_filename(const gchar *buf){
    ...
    if (locale_filename)    {
        if (g_str_has_suffix(locale_filename, ".geany"))        {
            if (project_ask_close())                main_load_project_from_command_line(locale_filename, TRUE);
        }        else {
...
static gint socket_fd_gets(gint fd, gchar *buf, gint len){
    ...
    if (--len < 1)
        return -1;
    do    {
        if ((n = socket_fd_recv(fd, bp, len, MSG_PEEK)) <= 0)            return -1;
        if ((newline = memchr(bp, '\n', n)) != NULL)            n = newline - bp + 1;
        if ((n = socket_fd_read(fd, bp, n)) < 0)            return -1;
        bp += n;
        len -= n;
    } while (! newline && len);
...
gint socket_fd_write(gint fd, const gchar *buf, gint len){
    if (socket_fd_check_io(fd, G_IO_OUT) < 0)        return -1;

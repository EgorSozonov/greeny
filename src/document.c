...
GeanyDocument* document_find_by_real_path(const gchar *realname) {
    ...
    for (i = 0; i < documents_array->len; i++) {
        ...
        if (! doc->is_valid || ! doc->real_path) continue;
...
GeanyDocument *document_find_by_filename(const gchar *utf8_filename) {
    ...
    for (i = 0; i < documents_array->len; i++) {
        ...
        if (! doc->is_valid || doc->file_name == NULL) continue;
...
GeanyDocument *document_find_by_sci(ScintillaObject *sci) {
    ...
    for (i = 0; i < documents_array->len; i++) {
        if (documents[i]->is_valid && documents[i]->editor->sci == sci) return documents[i];
    }
    return NULL;
}
...
static void monitor_file_changed_cb(G_GNUC_UNUSED GFileMonitor *monitor, G_GNUC_UNUSED GFile *file,
    ...
    if (file_prefs.disk_check_timeout == 0) return;
    ...
    if (doc->priv->file_disk_status != FILE_OK)    ui_update_tab_status(doc);    
}
#endif
...
gboolean document_remove_page(guint page_num) {
    ...
    if (done && ui_prefs.new_document_after_close) document_new_file_if_non_open();
...
void document_apply_indent_settings(GeanyDocument *doc) {
    ...
    if (iprefs->detect_width && detect_indent_width(doc->editor, type, &width)) {
        if (width != iprefs->width) {
            ui_set_statusbar(TRUE, _("Setting indentation width to %d for %s."), width, DOC_FILENAME(doc));
        }
...
void document_show_tab(GeanyDocument *doc) {
    gtk_notebook_set_current_page(GTK_NOTEBOOK(main_widgets.notebook), document_get_notebook_page(doc));
}
...
GeanyDocument *document_open_file_full(GeanyDocument *doc, const gchar *filename, gint pos,
    ...
    if (reload || doc == NULL) {    /* doc possibly changed */
...
gboolean document_reload_prompt(GeanyDocument *doc, const gchar *forced_enc) {
    ...
    if (doc->file_name == NULL) return FALSE;

    if (forced_enc == NULL) forced_enc = doc->encoding;
...
void document_rename_file(GeanyDocument *doc, const gchar *new_filename) {
    ...
    if (result != 0) {
        dialogs_show_msgbox_with_secondary(GTK_MESSAGE_ERROR, _("Error renaming file."), g_strerror(errno));
    }
...
static void protect_document(GeanyDocument *doc) {
    ...
    if (!doc->priv->protected++) sci_set_readonly(doc->editor->sci, TRUE);
...
static void unprotect_document(GeanyDocument *doc) {
    ...
    if (!--doc->priv->protected && doc->readonly == FALSE) sci_set_readonly(doc->editor->sci, FALSE);
...
gboolean document_save_file_as(GeanyDocument *doc, const gchar *utf8_fname) {
    ...
    if (utf8_fname != NULL) SETPTR(doc->file_name, g_strdup(utf8_fname));
    ...
    if (ret) ui_add_recent_document(doc);
    return ret;
}
...
gboolean document_save_file(GeanyDocument *doc, gboolean force) {
    ...
    if (!force && !doc->changed) return FALSE;
    if (doc->readonly) {
        ui_set_statusbar(TRUE,
    ...
    if (! main_status.quitting)    {
        ...
        if (file_prefs.disk_check_timeout > 0) document_update_timestamp(doc, locale_filename);
...
gint document_find_text(GeanyDocument *doc, const gchar *text, const gchar *original_text,
    ...
    if ((selection_end - selection_start) > 0) { /* there's a selection so go to the end */
        if (search_backwards)
    ...
    if (search_pos != -1) {
        ...
        if (scroll) doc->editor->scroll_percent = 0.3F;
    } else {
...
gint document_replace_text(GeanyDocument *doc, const gchar *find_text, const gchar *original_find_text,
    ...
    if (search_pos != selection_start) {
        if (search_pos != -1) geany_match_info_free(match);
        return -1;
    }
...
document_replace_range(GeanyDocument *doc, const gchar *find_text, const gchar *replace_text,
    ...
    if (count > 0) {    /* scroll last match in view, will destroy the existing selection */
        if (scroll_to_match)
...
void document_replace_sel(GeanyDocument *doc, const gchar *find_text, const gchar *replace_text,
    ...
    if (selection_mode == SC_SEL_RECTANGLE && selected_lines > 1) {
        ...
        if (selection_end != -1) replaced = TRUE;
    }

    if (replaced) {    /* update the selection for the new endpoint */
...
static void document_load_config(GeanyDocument *doc, GeanyFiletype *type,
    ...
    if (filetype_changed) {
        ...
        if (type->id != GEANY_FILETYPES_NONE) symbols_global_tags_loaded(type->id);
...
void document_set_filetype(GeanyDocument *doc, GeanyFiletype *type) {
    ...
    g_return_if_fail(doc);
    if (type == NULL) type = filetypes[GEANY_FILETYPES_NONE];
...
void document_undo_clear(GeanyDocument *doc) {
    ...
    if (! main_status.quitting && doc->editor != NULL) document_set_text_changed(doc, FALSE);
}
...
void document_undo_add_internal(GeanyDocument *doc, guint type, gpointer data) {
    ...
    if (type != UNDO_SCINTILLA || !doc->changed) document_set_text_changed(doc, TRUE);
...
gboolean document_account_for_unsaved(void){
    ...
    for (p = 0; p < page_count; p++) {
        ...
        if (DOC_VALID(doc) && doc->changed) {
            if (! dialogs_show_unsaved_file(doc)) return FALSE;
        }
    }
...
gboolean document_close_all(void) {
    if (! document_account_for_unsaved()) return FALSE;

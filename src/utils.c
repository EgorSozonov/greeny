...
const gchar *utils_find_open_xml_tag_pos(const gchar sel[], gint size) {
    ...
    if (G_UNLIKELY(size < 3)) {    /* Smallest tag is "<p>" which is 3 characters */
        return NULL;
    }
    begin = &sel[0];
    cur = &sel[size - 1];
...
gchar *utils_strv_find_lcs(gchar **strv, gssize strv_len, const gchar *delim){
    ...
    foreach_str(_sub, first)    {
        ...
        for (n_chars = 1; n_chars <= chars_left; n_chars++) {
            if (NZV(delim))      {    /* ... and advance to the next delim char at the end, if any */
...
gchar **utils_strv_shorten_file_list(gchar **file_names, gssize file_names_len){
    ...
    for (i = 0; i < num; i++)    {
        if (lcs_len == 0)        {    /* no lcs, copy without prefix */
...
GDate *utils_parse_date(const gchar *input){
    ...
    if (g_date_valid(date))        return date;

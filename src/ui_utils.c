...
static void insert_include(GeanyDocument *doc, gint pos, const gchar *include) {
    ...
    g_free(text);
    if (pos_after >= 0) sci_goto_pos(doc->editor->sci, pos_after, FALSE);
}
...
void ui_add_recent_document(GeanyDocument *doc){
    ...
    if (! rdata.mime_type)        rdata.mime_type = (gchar *) "text/plain";
...
void uiInitBuilder(void){
    ...
    interface_file = g_build_filename(g_get_current_dir(), "geany.glade", NULL);
